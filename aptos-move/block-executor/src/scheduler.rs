...
pub struct Scheduler {
    ...
    commit_and_gas: Mutex<(usize, u64)>,
    is_first: AtomicBool,
}
...
impl Scheduler {
    pub fn new(num_txns: usize) -> Self {
        Self {
            ...
            txn_status: (0..num_txns)
                ...
                .collect(),
            max_gas: 52845, //u64::MAX,
            num_commit: AtomicUsize::new(0),
            commit_and_gas: Mutex::new((0, 0)),
    ...
    pub fn try_commit(&self, txn_idx: usize, txn_gas: u64) -> SchedulerTask {
        // println!("try commit {} gas {}", txn_idx, txn_gas);
        let mut cmt_and_gas = self.commit_and_gas.lock();
        {
            if (*cmt_and_gas).0 != txn_idx {
                return SchedulerTask::NoTask;
            }
            (*cmt_and_gas).0 += 1;
            (*cmt_and_gas).1 += txn_gas;
            let cmt = (*cmt_and_gas).0;
            let gas = (*cmt_and_gas).1;
            self.increase_commit_idx(cmt);
            ...
            if cmt == self.num_txn_to_execute() {
                println!(
                    "thread {} set done local_cmt_idx {}",
                    rayon::current_thread_index().unwrap(),
                    cmt
            ...
            match self.produce_validation_task(txn_idx + 1) {
                Some((version, guard)) => {
    ...
    pub fn next_task(&self) -> SchedulerTask {
        loop {
            ...
            // println!("id {} validate {} execute {}", rayon::current_thread_index().unwrap(), idx_to_validate, idx_to_execute);
